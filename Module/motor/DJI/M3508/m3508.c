...
static m3508_instance m3508[2];

void get_m3508_measure(const uint8_t* rx_data, uint32_t id, void* arg);
M3508_Status_t m3508_init(CAN_HandleTypeDef *hcan, const uint32_t txid, const uint8_t num)
{
    if (num > M3508_MAXNUM)
        return M3508_ERROR_INVALID_PARAM;
    if (txid == M3508_TX_1 || txid == M3508_TX_2)
    {
        const uint8_t index = (txid == M3508_TX_1) ? 0 : 1;
        m3508[index].can_ins = BSP_CAN_Init(hcan);
        m3508[index].num = num;
        m3508[index].txid = txid;

        if (txid == M3508_TX_1)
            for (uint8_t i = 0; i < num; i++)
            {
                BSP_CAN_RegisterStdCallback(m3508[index].can_ins, M3508_RX_1+i, get_m3508_measure, &m3508[index].ecd[i]);
            }
        else
            for (uint8_t i = 0; i < num; i++)
            {
                BSP_CAN_RegisterStdCallback(m3508[index].can_ins, M3508_RX_2+i, get_m3508_measure, &m3508[index].ecd[i]);
            }
        m3508[index].init = 1;
        return M3508_OK;
    }
...
{
    if (m3508_ins == NULL || !m3508_ins->init)
        return M3508_ERROR;
...
m3508_instance *Get_M3508_Ptr(const uint16_t txid)
{
    if (txid == M3508_TX_1)
        return &m3508[0];
    if (txid == M3508_TX_2)
